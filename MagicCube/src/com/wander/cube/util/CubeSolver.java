...
public class CubeSolver {
	...
	static List<Integer> moveList = new ArrayList<Integer>();
	static List<Integer> moveAmountList = new ArrayList<Integer>();
	...
	static private void setposition(int t, int n) {
		...
		case 5:// tetrad choice,parity,twist
			...
			n /= 6;
			for (; i < 8; i++, n >>= 1)
				pos[i + 12] = (char)(((n & 1) > 0) ? corn[offset + k++]
						- CHAROFFSET : j++);
			break;
				...
				if (tb[i] == l) {
					...
					for (int f = 0; f < 6; f++) {
						for (int q = 1; q < 4; q++) {
							...
							// if move as allowed in that phase, and position is
							// a new one
	...
	static public String GetResult(String sInput) {
		...
		for (; phase < 8; phase += 2) {
			for (j = 0; !searchphase(j, 0, 9); j++)
				;
			for (i = 0; i < j; i++) {
				sOutput += "FBRLUD".toCharArray()[move[i]] 
						+ "" + ((Integer)moveamount[i]).toString();
				sOutput += " ";
				moveList.add((Integer)move[i]);
				moveAmountList.add((Integer)moveamount[i]);
			}
		}
	...
	public static List<Integer> getMoveList() {
		return moveList;
	}

	public static List<Integer> getMoveAmountList() {
		return moveAmountList;
	}

	public static void main(String[] argv){
		String sInput = "LF UR UB UL RF DR DB DL FU FD BR BL LFU URB UBL LDF RUF RFD DLB DBR";
		String result = CubeSolver.GetResult(sInput);
